...
solution to the problem of package management, while exposing the package
dependency model via an API that can be consumed by a more opinionated build
stack. There are no system wide dependencies, no dependencies are shared
between different apps, and the dependency tree is flat.

Bower runs over Git, and is package-agnostic. A package may contain any type of
...
bower install
# Using a path to a local folder
...
* A Git endpoint, e.g., `git://github.com/someone/some-package.git`. Can be
  public or private. ‡
* A shorthand endpoint, e.g., `someone/some-package` (defaults to GitHub). ‡
* A URL to a file, including `zip` and `tar.gz` files.
...
The easiest approach is to use Bower statically, just reference the packages
manually from a script tag:
...
* `directory`: Set the default directory to install packages into.
...
private registry of components while also taking advantage of public Bower
registries. If a component is not found at your private endpoint, Bower will
...
**N.B.** To run your own Bower Endpoint for custom components/packages that are
...
easily consume Bower components.
